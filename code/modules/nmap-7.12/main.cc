...
//extern "C" {
//#endif
int scan_main(input_host *input_argv, scan_callback cb) {
  ...
  */
  ...
  set_program_name(input_argv->argv[0]);
  ...
  if ((cptr = getenv("NMAP_ARGS"))) {
    ...
    for (i = 1; i < input_argv->argc && strlen(command) + strlen(input_argv->argv[i]) + 1 < sizeof(command); i++) {
      strcat(command, " ");
      strcat(command, input_argv->argv[i]);
    }
    myargc = arg_parse(command, &myargv);
    if (myargc < 1) {
      fatal("NMAP_ARGS variable could not be parsed");
    }
    ret = nmap_main(myargc, myargv, input_argv, cb);
    arg_parse_free(myargv);
    return ret;
  }

  if (input_argv->argc == 3 && strcmp("--resume", input_argv->argv[1]) == 0) {
    /* OK, they want to resume an aborted scan given the log file specified.
       Lets gather our state from the log file */
    if (gather_logfile_resumption_state(input_argv->argv[2], &myargc, &myargv) == -1) {
      fatal("Cannot resume from (supposed) log file %s", input_argv->argv[2]);
    }
    return nmap_main(myargc, myargv, input_argv, cb);
  }

  return nmap_main(input_argv->argc, input_argv->argv, input_argv, cb);
}
//#ifdef __cplusplus
//}
//#endif

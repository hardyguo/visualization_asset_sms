...
int nmap_main(int argc, char *argv[], input_host *input_argv, scan_callback scan_cb) {
  int i;
  ...
  do {
    ...
    while (Targets.size() < ideal_scan_group_sz) {
      ...
      /*output host discovery info by callback begin*/
      memset(&host_info, 0, sizeof(output_host));
      if(currenths->TargetName())
        host_info.name = currenths->TargetName();
      else
        host_info.name = currenths->HostName();
      host_info.targetipstr = currenths->targetipstr();
      host_info.status = currenths->flags;
      const u8 *mac = currenths->MACAddress();
      if (mac){
          Snprintf(host_info.macascii, sizeof(host_info.macascii), "%02X:%02X:%02X:%02X:%02X:%02X",
                       mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
          host_info.macvendor = MACPrefix2Corp(mac);
      }
      scan_cb(input_argv, &host_info);

      /*output host discovery info by callback end*/
